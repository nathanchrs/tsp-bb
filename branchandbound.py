...
# Input:
# - current_lower_bound: cost lower bound of input_graph
# - input_graph: adjacency matrix
# Returns (cost lower bound, reduced adjacency matrix (cloned))
def reduced_cost_matrix(current_lower_bound, input_graph):
	row, col = input_graph.shape
	...
	#print output_graph # debug
	#print current_lower_bound
...
def complete_tour_cost(current_lower_bound, input_graph):
	return 0, copy.deep_copy(input_graph)
...
def calculate_cycle_distance(graph, nodes_visited):
	distance = 0
...
def tsp_branch_and_bound(input_graph, bounding_function):
	...
	insert_index = 0
	start_lower_bound, start_graph = bounding_function(0, input_graph)
	...
	while not pq.empty():
		...
		# debug
		#print 'aaaaa'
		#print (current_lower_bound, current_graph, steps)
		...
		if len(steps) == graph_n and input_graph[steps[-1], 0] >= 0:
			steps.append(0)
			cycle_distance = calculate_cycle_distance(input_graph, steps)
		...
		for next_node in range(0, graph_n):
			if current_graph[steps[-1], next_node] >= 0:
				...
				next_lower_bound, next_graph = bounding_function(current_lower_bound, next_graph)
